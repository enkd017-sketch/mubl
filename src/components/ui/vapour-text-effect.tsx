...
export default function VaporizeTextCycle({
  ...
  const fontConfig = useMemo(() => {
    ...
    const multipliedSpread = vaporizeSpread * spread;
      ...
      switch (animationState) {
        ...
        case "fadingIn": {
          ...
          particlesRef.current.forEach((particle) => {
            ...
            const fill = particle.color.replace(/[\d.]+\)$/, `${opacity})`);
            ctx.fillStyle = fill;
    ...
    if (shouldVaporize) {
      ...
      if (particle.shouldFadeQuickly) {
        ...
        const dampingFactor = Math.max(
          0.95,
          1 - distanceFromOrigin / (100 * multipliedSpread),
        );
...
const renderParticles = (
  ...
  particles.forEach((particle) => {
    if (particle.opacity > 0) {
      const fill = particle.color.replace(/[\d.]+\)$/, `${particle.opacity})`);
      ctx.fillStyle = fill;
      ctx.fillRect(particle.x / globalDpr, particle.y / globalDpr, 1, 1);
    }
  });
